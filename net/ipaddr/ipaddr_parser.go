...
import (
	...
	"github.com/abmpio/libx/net/template"
)
...
func (t *IPAddrTemplate) ExpandAddrs(name string, s *string) []net.Addr {
	...
	if err != nil {
		t.Err = fmt.Errorf("%s: error parsing %q: %s", name, *s, err.Error())
		return nil
	}
	...
	for _, a := range strings.Fields(x) {
		...
		default:
			...
			if ip == nil {
				t.Err = fmt.Errorf("%s: invalid ip address: %s", name, a)
				return nil
			}
...
func (t *IPAddrTemplate) ExpandFirstAddr(name string, s *string) net.Addr {
	...
	if len(addrs) > 1 {
		...
		t.Err = fmt.Errorf("%s: 发现多个ip地址: %s", name, strings.Join(x, " "))
		return nil
	}
	return addrs[0]
}
...
func (t *IPAddrTemplate) ExpandFirstIP(name string, s *string) *net.IPAddr {
	...
	case *net.IPAddr:
		return a
	case *net.UnixAddr:
		t.Err = fmt.Errorf("%s cannot be a unix socket", name)
		return nil
	default:
		t.Err = fmt.Errorf("%s has invalid address type %T", name, a)
		return nil
	}
}
